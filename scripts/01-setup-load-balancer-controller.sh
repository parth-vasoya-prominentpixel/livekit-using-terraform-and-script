...
if aws iam get-policy --policy-arn "$POLICY_ARN" >/dev/null 2>&1; then
    ...
    # Add missing permissions to the policy
    echo "📋 Adding missing ELB permissions to policy..."
    cat > iam_policy_complete.json << 'EOF'
{
    "Version": "2012-10-17",
    "Statement": [
        {
...
EOF
    ...
    # Create IAM policy with complete permissions
    echo "📋 Creating IAM policy with complete permissions..."
    aws iam create-policy \
        --policy-name "$POLICY_NAME" \
        --policy-document file://iam_policy_complete.json
    
    # Clean up
    rm -f iam_policy.json iam_policy_complete.json
